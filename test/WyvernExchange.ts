...
describe("WyvernExchange", () => {
    ...
    const abi = [{
        ...
        'inputs': [{'name': 'addrs', 'type': 'address[]'}, {
            'name': 'values',
            'type': 'uint256[]'
        }, {'name': 'calldataLengths', 'type': 'uint256[]'}, {'name': 'calldatas', 'type': 'bytes'}],
        'name': 'atomicize',
        'outputs': [],
        'payable': false,
    ...
    describe("atomicMatch", () => {
        ...
        it('matches nft-nft swap order', async () => {
            ...
            let proxy6 = await registry.connect(accounts[6]).proxies(accounts[6].address)
            await erc721.connect(accounts[6]).setApprovalForAll(proxy6, true)
            await erc721.transferFrom(accounts[0].address, accounts[6].address, nfts[1])

            const selector = statici.interface.getSighash('swapOneForOneERC721')
            const paramsOne = statici.interface._abiCoder.encode(
                ['address[2]', 'uint256[2]'],
                [[erc721.address, erc721.address], [nfts[0], nfts[1]]]
            )
            const paramsTwo = statici.interface._abiCoder.encode(
                ['address[2]', 'uint256[2]'],
            ...
            const firstData = erc721.interface.encodeFunctionData('transferFrom', [accounts[0].address, accounts[6].address, nfts[0]])
            const secondData = erc721.interface.encodeFunctionData('transferFrom', [accounts[6].address, accounts[0].address, nfts[1]])
            ...
            expect(await erc721.ownerOf(nfts[0])).to.be.equal(accounts[6].address, 'Incorrect owner')
            expect(await erc721.ownerOf(nfts[1])).to.be.equal(accounts[0].address, 'Incorrect owner')
        })
    })
})
